...
export default async function CohortPage() {
      ...
      {stalled === 0 ? (
        ...
        <ul className="mt-4 space-y-3">
          {rows
            ...
            .map((r) => (
              <li key={r.member_id}>
                <Link href={`/cohort/${r.member_id}`} className="text-small hover:text-accent-text">
                  {r.full_name}
                </Link>
                <p className="text-tiny text-muted">
                  {copy.cohort.railDays(r.days_since_activity)}
                  {r.overdue_assignments ? ` · ${copy.cohort.railOverdue(r.overdue_assignments)}` : ""}
                </p>
              </li>
            ))}
        </ul>
  ...
  return (
    <StaffShell profile={me} rail={rail}>
      ...
      {/* Cohort map: one column per module */}
